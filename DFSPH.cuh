...
{
	{
		...
		//Real max_stiffness;		
		Real max_rho_err, max_dp_err;
		Real reference_density = 1000.;
		int iteration_DI, iteration_DF;
		...
		initialize_DFSPH(particle_array, dt, time);
		...
		try	// Exception handling											
		{
			...
			while (time < time_end)
			{
				...
				//		// calculate pressure by Density Invariant solver
				//		particle_array.calculate_pressure_DFSPH(dt);

				//		// check pressure convergence criterion
				//		max_rho_err = *(thrust::max_element(thrust_rho_err, thrust_rho_err + number_of_particles));

				//		if (iteration_DI >= minIteration)
				//		{
				...
				//		// calculate Density Invariant solver
				//		particle_array.density_invariant_solver_DFSPH(dt);

				//		// check density convergence criteria
				//		max_rho_err = *(thrust::max_element(thrust_rho_err, thrust_rho_err + number_of_particles));
			...
			outofDILoop:;
				...
				//-------------------------------------------------------------------------------------------------
				// ##. TIME INTEGRATION: UPDATE POSITION (DFSPH)
				//-------------------------------------------------------------------------------------------------
				...
				//-------------------------------------------------------------------------------------------------
				// ##. DIVERGENCE - FREE (DFSPH)
				//-------------------------------------------------------------------------------------------------

				// Divergence- Free Loop
				iteration_DF = 0;

				for (int k = 0; k < maxIteration - 2; k++)
				{
					iteration_DF++;

					// calculate Divergence free solver
					particle_array.divergence_free_solver_DFSPH(dt);

					// check density convergence criteria
					max_rho_err = *(thrust::max_element(thrust_rho_err, thrust_rho_err + number_of_particles));

					if (iteration_DF >= minIteration - 1)
					{
						if (max_rho_err < reference_density * drho_th)
						{
							goto outofDFLoop;
			...
			outofDFLoop:;
				...
				//-------------------------------------------------------------------------------------------------
				// ##. TIME INTEGRATION: UPDATE VELOCITY (DFSPH)
				//-------------------------------------------------------------------------------------------------
				...
				if (((count - 1) % freq_output) == 0)
				{
					...
					cout << "Number of DFSPH DF convergence iterations : Density Invariant =  " << iteration_DI << "  //   Divergence Free =  " << iteration_DF << endl;
...
if (solv.con_solve == 1)
{
//	clc_Conduction(particle, solv, number_of_particles, dim);
}
